...
// Função para obter informações da imagem com cache
async function getCachedImageInfo(id) {
  const cacheKey = `imgInfo_${id}`;
  ...
  try {
    ...
    for (const [index, image] of imagesToShow.entries()) {
      ...
      img.addEventListener("click", () =>
        openLightbox(image.download_url, images, startIndex + index),
      );

      // Busca as informações da imagem e adiciona o tooltip
      try {
        const info = await getCachedImageInfo(image.id);
        if (info) {
          addTooltip(img, info);
          // Armazena o nome do autor no data-attribute
          clonedContainer.querySelector(".image-container").dataset.author =
...
async function fetchImageInfo(id) {
  try {
    ...
    if (!response.ok) throw new Error("Erro ao buscar informações da imagem");
...
async function filterImages(authorName) {
  ...
  if (searchTerm === "") {
    ...
    console.log(
      "Pesquisa vazia, mostrando todas as imagens:",
    ...
    console.log(
      `Filtro por "${searchTerm}" encontrou ${filteredImages.length} imagens`,
    );
  }
...
function populatePageContainer(images = allImages) {
  ...
  if (images.length === 0) {
    ...
    grid.innerHTML =
      "<p class='no-results'>Nenhuma imagem encontrada com este autor.</p>";
    return;
  }
